...
TODO NOW:
7. Fix the issue where they didnt post at all bc they didnt exist counting towards a 0 post week.
8. Lable the graphs nicely and stuff

9. Perform Phi coeffieicnet correlation test on this. correlation, _ = pointbiserialr(df['Shootings'], df['Week_posts'])
print("Point-Biserial Correlation:", correlation)
10. Do the same correlation adn cross tabs with controls for the weighted duration, and then accounting for the engagment sum, and then both together. 
'''
...
def parse_csvs(csv_files, date_ranges):
    results = [[0, 0, 0, 0, 0] for _ in date_ranges]
    num_users = [0 for _ in date_ranges]
    for file in csv_files:
        df = pd.read_csv(file)
        has_post = [False for _ in date_ranges]
        for _, row in df.iterrows():
            ...
            results[week][3] += (row['View Count'] + row['Like Count'] + row['Comment Count'] + row['Favorite Count'])
            results[week][4] += (row['View Count'] + row['Like Count'] + row['Comment Count'] + row['Favorite Count']) * parse_video_duration(row['Duration'])
        started_posting = False
        for i in range(len(has_post)):
            if has_post[i]:
                started_posting = True
            if started_posting:
                num_users[i] += 1
    first_nonzero_week = [i for i in range(len(num_users)) if num_users[i]][0]
    return pd.DataFrame([[date, *(x/(users if users else 1) for x in r)] for date, r, users in zip(date_ranges[first_nonzero_week:], results[first_nonzero_week:], num_users[first_nonzero_week:])],
                        columns=['Start Date', 'Total Posts', 'Total Video Minutes', 'Total Views', 'Total Interactions', 'Total Interactions * Duration'])
...
# Column should be one of 'Start Date', 'Total Posts', 'Total Video Minutes', 'Total Views', 'Total Interactions', 'Total Interactions * Duration'
def create_graph(name, groups, interval='week', column='Total Posts', add_shootings=False):
    ...
    for group_name, group_files in groups.items():
        ...
        plt.plot(x[-len(y):], y, label=group_name)
    if add_shootings:
        y = [0] * len(x)
...
def calculate_post_frequency(csv_files, date_ranges):
    ...
    for i in range(len(results)):
        if num_users[i] > 0:
            results[i][1] /= num_users[i]
    first_nonzero_week = [i for i in range(len(num_users)) if num_users[i]][0]
    df = pd.DataFrame(results[first_nonzero_week:], columns=['Start Date', 'Num Posts'])
...
def create_crosstab(df, v1, v2, avg_views):
    crosstab = pd.crosstab(df[v1], df[v2])
    chi2, p, _, _ = chi2_contingency(crosstab)
    print("Chi-Square p-value:", p)

    # Calculate and print average value of v2 if v1 is True
    if True in crosstab.index:
        average_v2_when_true = df[df[v1] == True][v2].mean()
        # print("Average value of "+v2+" when "+v1+" is True:", average_v2_when_true)

    else:
        print("No True values in v1")
    if False in crosstab.index:
        average_v2_when_false = df[df[v1] == False][v2].mean()
        # print("Average value of "+v2+" when "+v1+" is False:", average_v2_when_false) 
        profit = str((average_v2_when_true - average_v2_when_false)*avg_views*0.001*2.5)
        print("Profit from Shootings: " + profit)
    else:
        print("No True values in v1")

    average_v2 = df[v2].mean()
    print("Average value of "+v2+":", average_v2)
...
def group_num_posts_big(num_posts):
    if num_posts == 0:
        return 0  # '0'
    elif num_posts <= 12:
        return 4  # '1-3'
        ...
        return 28  # '7-9'
    elif num_posts <= 80: # 10-20
        return 40
    else: 
        return 84 # '10+'
    
def group_num_views(num_posts):
    if num_posts == 0:
        return 0  # '0'
    elif num_posts <= 10000:
        return 10000  #  '4-6'
    elif num_posts <= 100000:
        return 100000  # '7-9'
    elif num_posts <= 1000000: # 10-20
        return 1000000
    elif num_posts <= 4000000: # 10-20
        return 4000000
    else: 
        return 6000000 # '10+'
    
def group_num_mins(num_posts):  #TODO
    if num_posts == 0:
        return 0  # '0'
    elif num_posts <= 30:
        return 10  # '1-3'
        ...
        return 70  # '7-9'
    elif num_posts <= 200: # 10-20
        return 100
    else: 
        return 210 # '10+'
    
def group_num_interactions(num_posts):  #TODO
    if num_posts == 0:
        return 0  # '0'
    elif num_posts <= 1000:
        return 500  # '1-3'
        ...
        return 300000  # '7-9'
    elif num_posts <= 500000: # 10-20
        return 750000
    else: 
        return 1000000 # '10+'
    
def group_num_weighted_interactions(num_posts): #TODO
    if num_posts == 0:
        return 0  # '0'
    elif num_posts <= 1000:
        return 500  # '1-3'
...
def main():
    ...
    # ben, CandaceOwens, JordanPeterson
    # Conservative_Focus_Group, Cons, Libs
    
    # 'Total Posts', 'Total Video Minutes', 'Total Views', 'Total Interactions * Duration'
    # VIEWS = 1591858.3797169812 #Peterson
    # VIEWS = 8230817 # Shapiro
    VIEWS = 496717.24855491327 # Owens
    COLUMN = 'Total Posts'  # 'Total Posts', 'Total Video Minutes', 'Total Views', 'Total Interactions', 'Total Interactions * Duration'
    GROUP_FUNC = group_num_posts
    GET_DATA = 'CandaceOwens'

    shootings_week_posts = parse_csvs(get_data(GET_DATA), get_date_ranges('week'))[['Start Date', COLUMN]]
    ...
    combined_df['Grouped'] = combined_df.apply(lambda row: GROUP_FUNC(row[COLUMN]), axis=1)

    # Create a crosstab and perform Chi-Square test
    # crosstab = 
    crosstab, p = create_crosstab(combined_df, 'Has Shootings', COLUMN, VIEWS)
    crosstab, p1 = create_crosstab(combined_df, 'Has Shootings', 'Grouped', VIEWS)
    p = "%.4f" % p
    p1 = "%.4f" % p1
    ...
    temp.plot()
    plt.title('JordanPeterson: CrossTab on Weekly Posts')
    plt.xlabel('    Number of Posts Per Week        p-value: ' + p1)
    plt.ylabel('Percent of Weeks where NumPosts within X axis range')
    # Save the plot
    #plt.savefig('Results/Posts/'+GET_DATA+'CTPosts.png')  # Save as PNG file
...
'''
 #Stuff to include in the write up:
 - Mention the P values are Chi squared tests. 
 - Mention what we want the shape of the graph to look like. the green line should be higher than the blue line towards the right, and when both of them (especially green) is a high percentage
 - Describe the exact cluster ranges on the graph so it is clear what that axis means
 - Display the avg additional videos per week and expain how much money that is.
 Chi-Square p-value: 0.008820451448915455 FOR Ben Shapiro
Average value of Total Posts when Has Shootings is True: 16.345794392523363
Average value of Total Posts when Has Shootings is False: 13.894160583941606
Diff is 2.45
Estimate $2.5 per thousand video views
Average value of Total Views: 8230817.128608924
(9899000-7579314= 2,319,836 /1000 * 2.5 = 5800)
2.45*8,230,817 = 20,165,501
/1000 * 2.5 = 50,414
107 weeks of shootings:
Total Profit Since 2017: $5,394,000
For reference, social blade calculates that is about how much he makes in a YEAR. He gets a whole year of income every 7 years, related to mass shootings.
...
- Note: Other consevrative channels are not correlated like the big 3. I checked all of them. Literally none of them have this behavior with significance.
- The liberals don't either. I did not check all, but a random sample. Same with the controls.
